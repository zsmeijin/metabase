...
(defn- prep-for-html-rendering'
  ...
  (let [results (#'body/prep-for-html-rendering pacific-tz {} {:cols cols :rows rows} bar-column min-value max-value (count cols))]
    [(first results)
     (col-counts results)]))
...
(deftest bar-column
  (is (= [{:bar-width (float 85.249),  :row [(number "1") (number "34.10") "Apr 1, 2014" "Stout Burgers & Beers"]}
         ...
         (rest (#'body/prep-for-html-rendering pacific-tz {} {:cols test-columns :rows test-data} second 0 40 (count test-columns))))))
