...
(s/defn ^:private query-results->row-seq
  ...
  [timezone-id :- (s/maybe s/Str) remapping-lookup cols rows bar-column min-value max-value]
  (for [row rows]
...
(s/defn ^:private prep-for-html-rendering
  ...
  HTML"
  ([timezone-id card data column-limit]
   (prep-for-html-rendering timezone-id card data nil nil nil column-limit))
  ([timezone-id :- (s/maybe s/Str) card {:keys [cols rows]} bar-column min-value max-value column-limit]
   (let [remapping-lookup (create-remapping-lookup cols)
         limited-cols (take column-limit cols)]
     (cons
      (query-results->header-row remapping-lookup card limited-cols bar-column)
      (query-results->row-seq timezone-id remapping-lookup limited-cols (take rows-limit rows) bar-column min-value max-value)))))
    ...
    {:attachments
     ...
     [:div
      (table/render-table (color/make-color-selector data (:visualization_settings card))
                          ...
                          (mapv :name cols)
                          (prep-for-html-rendering timezone-id card data y-axis-rowfn min-value max-value 2))
